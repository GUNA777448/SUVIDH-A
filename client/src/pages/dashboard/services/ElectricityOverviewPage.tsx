...
export default function ElectricityOverviewPage() {
  const navigate = useNavigate();
  const user = useAuthStore((state) => state.user);
  const token = useAuthStore((state) => state.token);
  const { getPendingBill } = useServiceBills();

  const pendingBill = getPendingBill("electricity");
  ...
  return (
    <main className="min-h-screen w-full overflow-x-hidden bg-[#131314] pb-24 pt-4 text-[#E3E3E3]">
      <div className="space-y-4 px-4">
        <h1 className="material-headline text-white">Electricity Services</h1>

        <section className="rounded-2xl border border-white/10 bg-[#1E1F20] p-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-[#2A2B2C] p-4">
              <p className="material-label text-slate-400">Current Bill</p>
            ...
            <div className="rounded-xl bg-[#2A2B2C] p-4">
              <p className="material-label text-slate-400">This Month</p>
              <p className="material-headline mt-2 text-white">235 kWh</p>
              <p className="material-label mt-1 text-slate-500">Usage</p>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-2">
          {[
            ...
            },
            {
              ...
              icon: TrendingDown,
              to: "/services/electricity/consumption",
            },
            {
              title: "Manage Connection",
              icon: Zap,
              to: "/services/electricity/connection",
            },
            {
              title: "Report Complaint",
              icon: AlertCircle,
              to: "/services/electricity/complaint",
            },
          ].map((item) => {
